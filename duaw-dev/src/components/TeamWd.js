        ...
        <Grid container spacing={{xs:2, md:2}} >
            ...
            <Grid item xs={2} md={2} style={{margin:"12px 0px"}}>
                <Box
                        ...
                        alt="Logo"
                        src="/TRACK-PINK-WD-WEB-DEVELOPMENT.svg"
                    />
                </Grid>
                <Grid item xs={10} md={10} >
                    <Typography variant='h6'style={{ marginLeft:"10px 0px 0px 10px", display:"block" }}>
                        {member.nameFull}
                    </Typography>
                    <IconButton 
...
const TeamWd = (props) => {
return props.teams.filter((item) => item.track==="WD" ).map((member)=>(
<Team key={member.id} member={member} />
)) ;
};

export default TeamWd;
